...
export default function Programmers() {
  ...
  const [progArr] = useState(listOfAwesome);
      ...
      <div className='programmers'>
        {
          ...
          progArr.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setActive(dev.id) /* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
